...
def ff_base_approximation_tanh_model(
    x: np.ndarray,
    r1: float,
    r2: float,
    c1: float,
...
def cross_fcn_sloped_tanh_model(
    ...
    for index in range(slopes_num):
        ...
        b = ff_base_approximation_tanh_model(
            x, left_r, right_r, left_c, right_c, s=s, hs=hs
        )
        slope_fcn += b

        fcn_bias += slope_val * ff_base_approximation_tanh_model(
...
def find_optimal_cross_tanh_model(
...
def analyse_cross_ff_tanh_model(
    hs: np.ndarray,
    S: np.ndarray,
    crossover_amount,
    ...
    change_cross_value = partial(
        cross_fcn_sloped_tanh_model, crossover_amount=crossover_amount
    )
    ...
    init_params = find_optimal_cross_tanh_model(
        hs, S, crossover_amount=crossover_amount
    )
    p0 = (
    ...
    def fit_func(x, *init_params):
        return cross_fcn_sloped_tanh_model(
            x,
            init_params[0],
            *init_params[1:],
